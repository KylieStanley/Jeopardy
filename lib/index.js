...
let submitWager = document.querySelector('.submit-wager');
...
gameBoard.addEventListener('click', handleCards);
...
function openQuestionCard(question) {
  ...
  domUpdates.showCardOptions(cardParent, card);
}

function openWagerCard(question) {
  domUpdates.showCardOptions(wagerParent, wager)
  userWager.style.display = 'inline-block';
  submitWager.style.display = 'inline-block';

  submitWager.addEventListener('click', function() {
  domUpdates.hideCardOptions(wagerParent, wager)
  // wagerParent.classList.add('hidden')
